Aquí tienes el texto perfectamente estructurado y limpio en formato Markdown, listo para que lo copies y pegues directamente en tu archivo `README.md`. He ajustado los bloques de código y la indentación para que se visualice correctamente en GitHub.
...
│
├── docs/
│   ├── informe/                # El documento Word del Informe Final y prototipos de Figma
│   └── diagramas/              # Imágenes de la arquitectura y capturas para los Anexos
│
├── sql/
│   └── hive/
│       └── create_tables.hql   # Scripts SQL para crear las tablas en Hive apuntando a la capa Gold
│
└── src/                        # Código fuente del proyecto
    ├── ingestion/
    │   └── bronze_ingestion.py # Script para subir el CSV crudo al HDFS (/datalake/bronze)
    │
    ├── processing/
    │   ├── silver_transformation.py # PySpark: Limpieza, manejo de nulos y duplicados (Capítulo 5)
    │   └── gold_aggregation.py      # PySpark: Agregaciones, KPIs de negocio y cálculos finales
    │
    └── utils/
        └── mongo_connector.py       # Funciones auxiliares para conectar y enviar la capa Gold a MongoDB
